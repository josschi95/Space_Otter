...
public class JellyBossBattle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        kingJelly.transform.position = new Vector3(0, 9, 0);
    ...
    private IEnumerator AreaAttackCoroutine()
    {
        ...
        while (t < timeToRun)
        {
            float rotation = bulletSpawnParent.eulerAngles.z;
            rotation += rotationSpeed * Time.deltaTime;
            bulletSpawnParent.rotation = Quaternion.Euler(0, 0, rotation);
            ...
            if (t2 <= 0)
            {
                for (int i = 0; i < bulletSpawnPositions.Length; i++)
                {
                    ...
                    bullet.GetComponent<IProjectile>()?.SetDamage(1, player.GetDimension());
                }
                t2 = attackDelay;
            }
